...
def test_init():
    assert (SampledParameter('test', 0., 1.) is not None)

def test_attributes():
    sp = SampledParameter('test', 0., 1.)
    assert sp.name == 'test'
    assert np.isclose(sp.loc, 0.)
    assert np.isclose(sp.width, 1.)
...
def test_member_unit_transform():
    sp = SampledParameter('test', 0., 1.)
    utrans = sp.unit_transform(0.)
    assert np.isclose(utrans, sp.loc)
    utrans = sp.unit_transform(1.)

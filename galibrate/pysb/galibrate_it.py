...
def prune_no_samples(parameters, no_sample):
    pruned_pars = [
        parameter for parameter in parameters if parameter[0].name not in no_sample
    ]
...
def update_with_Keq_samples(parameters, Keq_sample):
    ...
    pruned_pars = [
        parameter for parameter in parameters if parameter[0].name not in k_reps
    ]
    return pruned_pars
...
class GaoIt(object):
    ...
    def __call__(self, parameter, loc=None, width=None):
        ...
        if loc is None:
            if width is None:
                loc = np.log10(parameter.value) - 2.0
            else:
                loc = np.log10(parameter.value) - (width / 2.0)
        if width is None:
            width = 4.0
    ...
    def sampled_parameters(self):
        return [
            SampledParameter(name, self.parms[name][0], self.parms[name][1])
...
class GAlibrateIt(object):
    ...
    def __init__(
        ...
        if gao_it is not None:
            ...
            self._sampled_parameters = [
                SampledParameter(parm.name, *gao_it[parm.name])
    ...
    def __call__(
        self, gao_population_size=1000, gao_kwargs=None, fitness_type="norm_logpdf"
    ):
        ...
        # Construct the GAO
        gao = GAO(self._sampled_parameters, fitness, population_size, **gao_kwargs)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        metavar="model_file",
        type=str,
        help="The model.py file that you want run NS on.",
    )
    parser.add_argument(
        "output_path",
        metavar="output_path",
        type=str,
        help="The file location where you want to output the NS run script.",
    )
